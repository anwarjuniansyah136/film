...
const ListFilm = () => {
  ...
  if(!role){
    ...
    const handleUpdateFilm = async()=> {
      await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/update-film/${
          updatedFilms.id_film
        }`,
        {
          method: "PUT",
          headers: {
    ...
    const handleDeleteFilm = async(id_film) => {
      if (confirm("Apakah anda yakin akan menghapus produk ini?")) {
        await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/delete-film/${id_film}`, {
          method: "DELETE",
          headers: { "Authorization": `Bearer ${token}` },
        })
    ...
    const handleAddNewFilm = async () => {
      try {
        ...
        if(!filmExist){
          ...
          const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/post-film`,{
              method: "POST",
              body: formData,
              headers: { "Authorization": `Bearer ${token}` },
            }
