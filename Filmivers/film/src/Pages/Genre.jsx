...
const Genre = () => {
  ...
  useEffect(() => {
    const fetchGenres = async () => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_BASE_URL}/api/get-genre`,{
    ...
    const fetchFilms = async () => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_BASE_URL}/api/get-all-films`,{
            headers: { "Authorization": `Bearer ${token}` }
          }
        );
        const data = await response.json();
  ...
  if(!role){
    ...
    const handleButtonClick = async (id) => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_BASE_URL}/api/get-film-by-genre/${id}`,{
              headers: { "Authorization": `Bearer ${token}` }
          }
        );
        ...
        const data = await response.json();
        genres.map((item)=>{
          if(item.id_type === data[0].id_type){
    ...
    const handleAddGenre = async () => {
      ...
      try {
        const genreExist = getGenre.some((item) => item.genre === addGenre.genre);
        if(!genreExist){
          await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/post-genre`, {
            method: "POST",
    ...
    const handleUpdateGenre = async (id) => {
      if (!updateGenre || !updateGenre.genre) return; // Ensure genre is not empty
      try {
        await fetch(
          `${import.meta.env.VITE_API_BASE_URL}/api/update-genre/${id}`,
          {
            method: "PUT",
            headers: { "Content-Type": "application/json","Authorization": `Bearer ${token}` },
    ...
    const handleDeleteGenre = async (id) => {
      if (window.confirm("Are you sure you want to delete this genre?")) {
        try {
          await fetch(
            `${import.meta.env.VITE_API_BASE_URL}/api/delete-genre/${id}`,
            {
              method: "DELETE",
              headers:{"Authorization": `Bearer ${token}`}
            }
